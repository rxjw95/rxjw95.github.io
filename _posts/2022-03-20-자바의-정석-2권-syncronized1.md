...
멀티 스레드 환경에서 공유 자원에 대한 무분별한 접근으로 인해 예측치 못한 결과를 얻게 되는 문제가 발생한다.
...
```
위의 코드를 실행해보면 잘 동작하는 것처럼 보인다. 하지만 위의 코드는 문제점이 하나 있다.

`wait/notify`를 설명할 때 waiting thread pool 이라는 것을 설명했다.

`Table` 객체에서 wait()를 하면 그 대상 주체가 Customer인지 Cook인지 우리는 알 수 없다. 즉, waiting pool에 Customer와 Cook이 함게 있다는 뜻이다.

그래서 notify()를 호출할 때, 어느 스레드가 통지받았는지 알 수 없다.

만약, 음식의 수가 줄어들어 notify()를 호출해서 Cook thread에 통지가 갔다면 음식을 만들겠지만, 계속해서 Customer thread에게 통지된다면 어떻게 될까?

### 기아 현상과 경쟁 상태
위의 예시처럼 운이 안좋게도 Cook thread가 통지를 받지 못한 채 무한정 대기를 하게 되는 상태를 `기아 현상(starvation)`이라고 한다.

위의 코드를 예시로 해당 기아 현상을 막기 위해선 notifyAll()을 호출해서 전체 스레드를 깨우는 것이다.

notifyAll()로 문제를 해결했다고 생각했지만, Customer thread까지 통지를 받았기 때문에, 불필요하게 Cook thread와 락을 얻기 위해 경쟁하는 상황이 발생한다. (다른 컨텍스트에 있는 Customer와 Cook을 싸우게 되는 불필요한 상황)

이를 개선하기 위해서는 Customer와 Cook을 구별하여 통지하는 방법이 필요하다.

위의 방법은 다음 포스팅에서 알아본다.
